...
solve(prepared(Family,Noteqs,Nodes,Doms,Precs,NotUnifs,Relations,NodeNames,Colors,Ranks,TagOps,Unicities,Table,NodeList1),solution(UTree,Children,Table)):-
	...
	xmg_compiler:send(debug,' using unicity '),
	%%xmg_compiler:send(info,Unicities),
	xmg_unicity:post_unicities(Space,NodeList,IntVars,Unicities),
	...
	(
		...
		xmg_compiler:send(debug,' using colors '),
		xmg_colors:colors(Space,NodeList,Colors)
	    );
	    true
	),!,
	(
	     xmg_compiler:principle(rank) ->
	     (
	 	%use_module('syn/xmg_rank'),
		xmg_compiler:send(debug,' using rank '),
		%%xmg_compiler:send(info,Ranks),
	 	xmg_rank:ranks(Space,NodeList,IntVars,Ranks,RankRels)
	     );
	     true
	 ),!,
	...
	%use_module(xmg_tag),
	xmg_compiler:send(debug,' using tag '),
	%% xmg_compiler:send(info,TagOps),
	xmg_tag:post_tags(Space,NodeList,TagOps),
...
unify_node([N1],node(P2,F2,NN2),NodeList):-
	...
	%% xmg_compiler:send(info,' could not unify '),
	%% xmg_compiler:send(info,Name1),
	%% xmg_compiler:send(info,Name2),
	%% xmg_avm:print_avm(P1),
	%% xmg_avm:print_avm(P2),
	%% xmg_avm:print_avm(F1),
	%% xmg_avm:print_avm(F2),
	%% NN1=NN2,
	%% xmg_compiler:send(info,' not names '),
	%% P1=P2,
	%% xmg_compiler:send(info,' not props '),
	%% F1=F2,
...
%% 	xmg_compiler:send(info,' unification failed '),
%% 	xmg_avm:avm(F,AF),
%% 	xmg_avm:avm(F1,AF1),
...
%% 	F=F1,
%% 	xmg_compiler:send(info,' unified ! '),
