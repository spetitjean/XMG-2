...
{
	<syn>{
		node xR(color=red)[cat = v,bot = [inv = @{pos,neg},mode = @{ind,subj}, num = ?fY, pers= ?fZ, gen = ?fU,aux = ?fV ,aux-pass = ?fW,neg-adv = ?fG, neg-nom = ?fK]]{
			node xfoot(color=red,mark=foot)[cat =v,top=[inv = neg, mode = @{ind,subj}, num = ?fY, pers = ?fZ, gen = ?fU, aux = ?fV, aux-pass=?fW,neg-adv = ?fG,neg-nom = ?fK]]
			node xCl(color=red,mark=anchor)[cat = cl]
		}
	}
}
	...
	<syn>{
		node xR(color=red)[cat = s]{
			...
			node xSbar(color=red,mark=nadj)[cat = s]{
				...
				node xArg(color=red,mark = subst)[cat = s, top=[inv = neg, princ = neg, wh = -, mode = @{subj,ind}]]
			}
		}
	}
}
...
{
	<syn>{
		node xR(color=red)[cat = s,bot = [princ = pos] ]{
			node xHead(color=red,mark=anchor)[cat = c]
			node xFoot(color=red,mark=foot)[cat = s, top = [inv = neg, mode = subj, wh = -]]
		}
	}
}
...
{
	<syn>{
		node xR(color=red)[cat = s,bot=[inv = @{pos,neg},wh = -]]{
			...
			node xS(color=red,mark=nadj)[cat = s]{
				node xComp(color=red)[cat=c]{
 				...
 				node xFoot(color=red,mark=foot)[cat = s,top=[inv = neg,wh = -]]
			}
		}
...
{
   <syn>{
		node xtop(color = red, mark = anchor)[cat = s, bot = [wh = -, inv = neg, princ = neg]]{
