...
{
		<syn>{
			node xS[bot = [wh = -,inv = neu]]{
				node xVN [top = [inv = neg]]
...
{
	<syn>{
		node xS[bot =[inv = pos,mode = @{ind,subj}]]{
		     node xVN[top = [inv = pos]]
		}
	}
}
	...
	<syn>{
		node xS{
			node xTop(color=red)[cat=s]{
				...
				node xSubj(color=red,mark=subst)[cat=s,top=[mode = subj, inv = neg, wh = -, princ = neg]]
			}
			node xVN
		}
	}
	...
	?xSubj
{
	<syn>{
		node xS{
			node xSubj(color=red,mark=subst)[cat=s, top=[mode = inf, princ = neg, inv = neg, wh = -]]
			node xVN
		}
	}
}
	...
	?xX ?xComp ?xArg
{
	<syn>{
		node xtop(mark=nadj)[cat=s]{
			...
			node xArg(color = red)[cat = s,top=[wh = -,princ = neg, inv = neg]]
		}		
	}
}
...
{
	<syn>{
		node xtop[top=[mode=inf, princ = neg]];
		?xCtrollee = ?xtop;
		?xVNCtroller = ?xVN
	}
}
...
{
	<syn>{
		node xArg[top=[mode=inf,wh = -, princ = neg]];
		?xCtrollee = ?xArg;
		?xVNCtroller = ?xVN			
	}
}
	...
	CanonicalSententialXObjectWithComplementizerBounding[]
{
	...
	<syn>{
		node xArg(color = red)[cat = s,top=[wh = -,inv = neg,princ = neg]]
	}
}
...
{
	<syn>{
		node xtop [top =[wh = +,mode = @{inf,ind},princ = neg]];
		?xtop = ?xCtrollee
	}
}
	...
	?xSe ?xtop ?fX
{
	<syn>{
		node xSe(color=red,extracted= +)[cat = s]{
			node xtop(color=red)
			node xS[top=[wh = -,inv = @{pos,neu,neg}]] %false -> ask yannick (should be + | n for questions, + | - | n for clefts and relatives (I can't say that)   
		}
	}
}
...
{
	<syn>{
		node xSe[top = [princ = pos],bot=[wh = +,princ = ?fX, inv = ?fX]];
		node xS[top=[inv = ?fX]]
	}
}
